...
def create_trials_with_definitions(nwbfile):
    ...
    nwbfile.add_trial_column(
        name="HED", col_cls=HedTags, data=[], description="HED annotations using custom definitions"
    )
...
def create_events_with_definitions(nwbfile):
    ...
    events_table = DynamicTable(
        ...
        columns=[
            VectorData(name="event_time", description="Time of event", data=[1.5, 3.2, 7.8, 10.1, 13.4]),
            VectorData(name="stimulus_type", description="Type of stimulus", data=["go", "go", "stop", "go", "go"]),
            VectorData(
                name="response_type",
            ...
            VectorData(
                name="response_time", description="Response time in seconds", data=[0.45, 0.52, None, 0.89, 0.32]
            ),
            HedTags(
                data=[
