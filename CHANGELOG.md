...
- `HedNWBValidator.validate_file()` now performs **assembled** (BIDS-style) validation of every `DynamicTable` except `MeaningsTable`: each table is converted to a dataframe + sidecar and validated with hedtools `TabularInput`, which combines a row's direct/categorical/value HED into one annotation. If the table has an `onset` column the validation is temporal (timeline); otherwise it is non-temporal. A `MeaningsTable` is not validated on its own (its categorical HED is validated as part of the table whose column it annotates) and is checked only against the rule that it must not contain a `HedValueVector` column. (`validate_events()`, `validate_table()`, `validate_vector()`, and `validate_value_vector()` remain available; `validate_table`/`validate_vector`/`validate_value_vector` do per-column checks.)
...
- `validate_table()` now records the table in the hedtools `TABLE_NAME` error context (`ec_table_name`) instead of `FILE_NAME` (`ec_filename`); `get_printable_issue_string` prints it as "Errors in table '<name>'". `validate_file()` still uses `FILE_NAME` for the NWB file identifier.
- New `spec_tests/` directory runs the hed-tests validation suite (the sidecar, event, and combo cases of `validation_tests.json`; hed-tests is a git submodule) through `HedNWBValidator.validate_file`, converting each case to NWB objects with the library's own BIDS converters. Cases the NWB representation cannot hold, or that need a hedtools feature not yet available, are skipped with a stated reason in `spec_tests/skipped_cases.py`. Run with `python -m pytest spec_tests`; see `spec_tests/README.md`. The stale copy of `validation_tests.json` under `src/pynwb/tests/data/` was removed.
- `HedNWBValidator.validate_file()` and `validate_events()` now stop validating a table after any error in its sidecar-level HED (a `HedValueVector` template or a MeaningsTable HED string), not only after a structural sidecar error. Such an error would be repeated on every row that uses it and would make the assembled row annotations unreliable. The sidecar issues name the column and the categorical value (`ec_sidecarColumnName`, `ec_sidecarKeyName`) and have no row. Warnings alone do not stop validation. All issues from the assembled path now also carry the table name in `ec_table_name`.
